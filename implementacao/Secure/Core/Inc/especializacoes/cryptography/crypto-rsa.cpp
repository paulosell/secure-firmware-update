...
CryptoRSA::STATUS_t CryptoRSA::sigCheck(uint8_t *buffer_expected,
	...
	chave.mExponentSize = (int32_t) pubkey->rsa_key.exponent_len;
	chave.mModulusSize = (int32_t) pubkey->rsa_key.modulus_len;
	chave.pmExponent = pubkey->rsa_key.exponent;
	chave.pmModulus = pubkey->rsa_key.modulus;
